...
import { Search, Download, Filter, RefreshCw, CheckCircle, XCircle, AlertCircle, Calendar, DollarSign, FileText } from 'lucide-react';

const BankReconciliation = () => {
  const [reconciliations, setReconciliations] = useState([
    { id: 1, date: '2024-01-15', description: 'Client Payment', bookAmount: 125000, bankAmount: 125000, difference: 0, status: 'Matched' },
    { id: 2, date: '2024-01-14', description: 'Vendor Payment', bookAmount: 45000, bankAmount: 44500, difference: 500, status: 'Mismatch' },
    { id: 3, date: '2024-01-13', description: 'Salary Payment', bookAmount: 180000, bankAmount: 180000, difference: 0, status: 'Matched' }
  ]);
  ...
  const stats = [
    ...
    { title: 'Mismatched', value: mismatched.toString(), icon: XCircle, gradient: 'from-red-500 to-red-600' },
    { title: 'Total Difference', value: `₹${totalDifference.toLocaleString()}`, icon: DollarSign, gradient: 'from-orange-500 to-orange-600' },
    { title: 'Reconciliation Rate', value: `${Math.round((matched / reconciliations.length) * 100)}%`, icon: AlertCircle, gradient: 'from-blue-500 to-blue-600' }
  ];

  const getStatusConfig = (status) => {
    const configs = {
      'Matched': { color: 'bg-green-100 text-green-700 border-green-200', icon: CheckCircle },
      'Mismatch': { color: 'bg-red-100 text-red-700 border-red-200', icon: XCircle },
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 p-8">
      ...
      <div className="mb-8 flex flex-col md:flex-row md:items-center md:justify-between">
        <div>
          ...
          <p className="text-gray-600 text-lg font-medium">Reconcile bank statements with book entries</p>
        </div>
      ...
      <div className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden">
        <div className="bg-gradient-to-r from-blue-50 via-indigo-50 to-purple-50 px-8 py-6 border-b border-gray-200">
          ...
          <p className="text-sm text-gray-600 mt-1">Compare book entries with bank statements</p>
        </div>
          ...
          <table className="w-full">
            <thead className="bg-gradient-to-r from-gray-50 to-gray-100">
              <tr>
                ...
                <th className="px-6 py-4 text-right text-xs font-bold text-gray-700 uppercase tracking-wider">Book Amount</th>
                <th className="px-6 py-4 text-right text-xs font-bold text-gray-700 uppercase tracking-wider">Bank Amount</th>
                ...
                return (
                  <tr key={record.id} className="hover:bg-gradient-to-r hover:from-blue-50 hover:to-transparent transition-all duration-200">
                    ...
                    <td className="px-6 py-4">
                      <span className="font-bold text-gray-900">{record.description}</span>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <span className="font-bold text-gray-900">₹{record.bookAmount.toLocaleString()}</span>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <span className="font-bold text-blue-600">₹{record.bankAmount.toLocaleString()}</span>
                    </td>
                    <td className="px-6 py-4 text-right">
                      <span className={`font-bold ${record.difference === 0 ? 'text-green-600' : 'text-red-600'}`}>
                        {record.difference === 0 ? '₹0' : `₹${Math.abs(record.difference).toLocaleString()}`}
                      </span>
                    </td>
